(function (global, factory) {
  ...
  (global = global || self, factory(global.laravelMix));
}(this, (function (mix) {
  ...
  class ReactCSSModules {
     ...
     */
    constructor() {
      this.replaceLoaderOptions = rule => {
        ...
        rule = typeof rule === "string" ? {
          rule
        } : rule; // Inject our options into the loader

        rule.options = rule.options ? Object.assign({}, rule.options, options) : options;
        return rule;
      };
    ...
    webpackConfig(config) {
      ...
      config.module.rules = config.module.rules.map(rule => {
        ...
        rule.use = Array.isArray(rule.use) ? rule.use.map(this.replaceLoaderOptions) : rule.use; // Alternatively, loop through all of the oneOf options (if they exist)

        rule.oneOf = Array.isArray(rule.oneOf) // And then replace all the loaders on each individual option
